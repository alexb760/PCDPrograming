...
//                    startIndexInclusive, mid, input);
//                ReciprocalArraySumTask right = new ReciprocalArraySumTask(
//                    mid, endIndexExclusive, input);
//                invokeAll(left, right);
//                value = left.getValue() + right.value;
//            }
        }
    }
...
//        System.setProperty("java.util.concurrent.ForkJoin.common.parallelism","3");
//
//        ForkJoinPool.commonPool().invoke(task);
    ...
    System.out.println(numTasks);
        List<ReciprocalArraySumTask> chunkTask = new ArrayList<>();
        for (int i = 0; i < numTasks; i++){
            chunkTask.add(new ReciprocalArraySumTask(
...
//        fjp.invokeAll(chunkTask);
        ForkJoinTask.invokeAll(chunkTask);

        return chunkTask.stream().mapToDouble(ReciprocalArraySumTask::getValue).sum();
    }
}
